...
public class SingleJoinService {
    ...
    public JoinResult getInfo() {
        ...
        int table_num_records = dbConnector.queryFor(table1_count_query).getColumn(0).getInt(0);
        int table2_num_records = dbConnector.queryFor(table2_count_query).getColumn(0).getInt(0);

        float table_success_rate, table2_success_rate;
        int combined_num_records;
        String completion;
        if (isComplete) {
            combined_num_records = dbConnector.queryFor(combined_count_query).getColumn(0).getInt(0);
